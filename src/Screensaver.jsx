...
export default function Screensaver({ onDismiss }) {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(stored)) setSnaps(stored)
    } catch { setSnaps([]) }
  }, [])

  // cycle through snapshots oldest→newest every 5s with a fade
  useEffect(() => {
  ...
  return (
    <div className="screensaver" onClick={onDismiss}>
      ...
      <div className="ss-branding">
        ...
        <div className="ss-tagline">WOLFSBURG · INTERFACE</div>
        {snaps.length > 0 && (
          <div className="ss-count">
            {idx + 1} / {snaps.length} &nbsp;·&nbsp; {snaps.length} view{snaps.length !== 1 ? 's' : ''} captured
          </div>
        )}
      </div>
