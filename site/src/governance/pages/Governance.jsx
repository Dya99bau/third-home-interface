...
import { proposals as seedProposals, roleConfig } from '../data'
import { daysUntil, cn } from '../utils'
...
function ProposalCard({ proposal, localState, canVote, onConsent, onConcern, onBlock, pending }) {
...
function NewProposalForm({ role, onSubmit, onCancel }) {
  const [title, setTitle] = useState('')
  const [description, setDescription] = useState('')
  const [category, setCategory] = useState('community')

  const categories = role === 'keeper' ? ['daily', 'community', 'structural'] : ['daily', 'community']
...
export default function GovernancePage() {
  ...
  function handleConcern(id, text) {
    setLocalStates((prev) => ({
      ...prev,
  ...
  function handleBlock(id, reason) {
    setLocalStates((prev) => ({
      ...prev,
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      {showNewForm && canPropose && (
        <div className="mb-6">
          <NewProposalForm role={role} onSubmit={handleNewProposal} onCancel={() => setShowNewForm(false)} />
        </div>
      )}
